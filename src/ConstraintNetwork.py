...
class ConstraintNetwork(nn.Module):
    ...
    def train(self, state, action, constraints_diff, epochs=25, batch_size = 256, split_ratio = 0.25):
       ...
       train_constraints = constraints_diff[train_idx,:] 
       ...
       for epoch in range(epochs):
           for batch in range(train_state.shape[0]//batch_size):
                ...
                constriants_diff_batch = torch.Tensor(train_constraint[batch_idx, :])
                ...
                loss = nn.MSELoss()(constraints_diff, dot_prod)
                loss.backward()
                self.optimizer.step()
...
def main():
    ...
    constraints_diff = pd.read_csv("DD_constraints_diff.csv").to_numpy()
    ...
    for i in range(N):
        ...
        net.train(states, actions, constraints_diff[:,i])
